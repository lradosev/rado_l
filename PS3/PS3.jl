...
# ╔═╡ b9c8a76b-e8cd-4b25-8361-1c712b792720
md"""
##### 3.2: Computation C2

Now that we have the metabolite data relevant to our problem, we should use that metabolite data and calculate the flux bounds.
"""
...
md"""
In part 4 - Calculating C3, we will modify the species bounds based on the effects of the specific growth rate on the available metabolites. 
"""
...
# ╔═╡ de5ffa6c-3d24-4ded-9570-1e666866f086
...
# ╔═╡ 2fd0a941-1470-4f95-a609-e9809ab35714
begin 
	# get flux values from the result -
	calculated_flux_vector_C1 = result_case_1.calculated_flux_array

	# build flux table -
	flux_table_C1 = Array{Any,2}(undef,ℛ,4)

	# populate -
	for i ∈ 1:ℛ
		flux_table_C1[i,1] = i
		flux_table_C1[i,2] = rna[i]
		flux_table_C1[i,3] = calculated_flux_vector_C1[i]*(1) # units: μmol/gDW-hr
		flux_table_C1[i,4] = expanded_reaction_array[i]
	end
end

# ╔═╡ d6277ced-807b-46e0-b15d-7ec58e833c50
calculated_flux_vector_C1
...
begin
	...
	S_C3 = S
	species_bounds_array_C3 = zeros(18,2)
	
	flux_vector_C3 = copy(calculated_flux_vector_C1)
	species_bounds_array_C3 = μ * conc_array/
	
	# compute the flux -
	result_case_3 = lib.flux(S_C3,flux_bounds_array_C3,species_bounds_array_C3,c_vector);

	# show -
	nothing
end

# ╔═╡ c0cfe2f9-a8fe-4f41-80c0-67a13d132b71
begin
	sk = df[:,6]
	#concentration = df[:,3]
	
	# calculate vi - the irreversible rate of reaction as it is bound by the metabolite data
	mk1 = sk[5]/((sk[5]+1)-1)
	# mk1 = sk[5]*sk[6]/((sk[5]+1)*(sk[6]+1)-1)
	mk2 = sk[3]*sk[4]/((sk[3]+1)*(sk[4]+1)-1)
	mk3 = sk[2]*sk[8]/((sk[2]+1)*(sk[8]+1)-1)
	mk4 = sk[7]*sk[9]/((sk[7]+1)*(sk[9]+1)-1)
	mk5 = sk[1]/((sk[1]+1)-1)
	mk6 = mk5
	
	# the flux bounds array for Case 2 is calculated by applying the equation from the multisubstrate kinetics given above to the "unmodified" flux data from Case 1. The Vmax is what is being modified, so we take the results from the first case, and subtract out our mk modifier that accounts for metabolism  
	flux_bounds_array_C2 = zeros(21,2)
	S_C2 = S	
	flux_bounds_array_C_2 = copy(calculated_flux_vector_C1)
	flux_bounds_array_C2[1,2] = flux_bounds_array_C_2[1]*mk1
	flux_bounds_array_C2[2,2] = flux_bounds_array_C_2[2]*mk2
	flux_bounds_array_C2[3,2] = flux_bounds_array_C_2[3]*mk3
	flux_bounds_array_C2[4,2] = flux_bounds_array_C_2[4]*mk4
	flux_bounds_array_C2[5,2] = flux_bounds_array_C_2[5]*mk5
	flux_bounds_array_C2[6,2] = flux_bounds_array_C_2[6]*mk6

	flux_boundz = flux_bounds_array - flux_bounds_array_C2
	
	# compute the flux -
	result_case_2 = lib.flux(S_C2,flux_boundz,species_bounds_array,c_vector);

	# show -
	nothing
end

# ╔═╡ ecf48f46-f7be-4abd-a188-d06e47690adf
# check:
with_terminal() do
...
# ╔═╡ 91925c1b-a029-48f3-a7fe-a00d0017046b
let
	# get flux values from the result -
...
# ╔═╡ eef1d043-5c8d-4874-aeba-9f22694b6f3d
begin 
	# get flux values from the result -
	calculated_flux_vector_C2 = result_case_2.calculated_flux_array

	# build flux table -
	flux_table_C2 = Array{Any,2}(undef,ℛ,4)

	# populate -
	for i ∈ 1:ℛ
		flux_table_C2[i,1] = i
		flux_table_C2[i,2] = rna[i]
		flux_table_C2[i,3] = calculated_flux_vector_C2[i]*(1) # units: μmol/gDW-hr
		flux_table_C2[i,4] = expanded_reaction_array[i]
	end
end
...
begin
	...
	# initialize -
	x_concentration_array3 = zeros(ℳ,2) # default: missing value = 0.0
	if (replacement_strategy_flag == 1)
		...
		x_concentration_array3 = (gmean_x)*ones(ℳ,2)*CF3 # units: μmol/gDW
	end
	...
	for (i,metabolite) ∈ enumerate(metabolites_in_our_model_array)
		...
		df_metabolite3 = filter([:Metabolite,:Organism]=>(x,y)->
			(x == metabolite && y=="Homo sapiens"), metabolite_table)
		
		# is the df empty?
		if (isempty(df_metabolite3) == false)
			x_concentration_array3[i,1] = df_metabolite3[1,:Concentration]
			x_concentration_array3[i,2] = CF3*df_metabolite3[1,:Concentration]
		else

			if (replacement_strategy_flag == 1)
				x_concentration_array3[i,1] = gmean_x
			end
		end
	end
end
...
let
	...
	for (i,metabolite) ∈ enumerate(metabolites_in_our_model_array)
		...
		state_array[i,3] = x_concentration_array3[i,1]
		state_array[i,4] = x_concentration_array3[i,2]
	end
...
let
	...
	# get flux values from the result -
	calculated_flux_array3 = result_case_3.calculated_flux_array
	if (size(calculated_flux_array,1) == ℛ)
		calculated_flux_array3 = [calculated_flux_array ; 0.0]
	end
	
	# build flux table -
	flux_table3 = Array{Any,2}(undef,(ℛ+1),4)

	rna = [rna ; "μ"]
	expanded_reaction_array3 = [expanded_reaction_array ; "precursors -> cellmass"]

	# populate -
	for i ∈ 1:(ℛ+1)
		flux_table3[i,1] = i
		flux_table3[i,2] = rna[i]
		flux_table3[i,3] = calculated_flux_array3[i]*(1) # units: μmol/gDW-hr
		flux_table3[i,4] = expanded_reaction_array3[i]
	end
...
# ╔═╡ 5e87af4e-fe34-47d9-adf9-d0bebb7e596f
begin

	# setup concentration array -
	conc_array4 = copy(x_concentration_array[:,2])
	
	# computation for C3 goes here ...
	S_C4 = S
	flux_bounds_array_C4 = flux_bounds_array_C3
	
	# compute the flux -
	result_case_4 = lib.flux(S_C4,flux_bounds_array_C4,species_bounds_array,c_vector);
